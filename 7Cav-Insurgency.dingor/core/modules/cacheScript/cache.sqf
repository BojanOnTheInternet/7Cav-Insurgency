...
if(_cityRadB>_cityRadA)then{_cityRadA=_cityRadB;};
_cacheHouses=[_cityPos,_cityRadA]call SL_findHouses;};
_tarHouse=_cacheHouses select(random((count _cacheHouses)-1));//Pull the array and select a random building from it
_cachePos=[_tarHouse]call SL_randHousePos;
sleep 1;
cache=createVehicle["Box_FIA_Wps_F",_cachePos,[],0,"NONE"];publicVariable "cache";cache setDamage 0;clearItemCargoGlobal cache;
//cache addEventHandler[
//		"handleDamage",
//		{if((_this select 4)=="SatchelCharge_Remote_Ammo")then{(_this select 0)setDamage 1;}else{(_this select 0)setDamage 0;};}
//	];
